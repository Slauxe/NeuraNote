...
import type { InfiniteBoard, PageTemplate, ShapePreset } from "@/lib/noteDocument";
...
export function useCanvasInteractions({
  ...
  const buildShapeDraft = useCallback((rawStart: Point, rawEnd: Point) => {
    const start = shapeSnapEnabledRef.current ? snapPoint(rawStart, true) : rawStart;
    let end = shapeSnapEnabledRef.current ? snapPoint(rawEnd, true) : rawEnd;
    if (shapePresetRef.current === "line" || shapePresetRef.current === "axis" || shapePresetRef.current === "axis-2d" || shapePresetRef.current === "axis-3d") {
    ...
    if (preset === "angle") {
      const marker = Math.min(Math.max(12, Math.min(absWidth, absHeight) * 0.32), 28);
      return {
    ...
    const xNegativeOriented = axisPoint(primaryUnit, -Math.max(20, dragLength * 0.35));
    const yPositive = axisPoint(secondaryUnit, Math.max(32, dragLength * 0.84));
    const yNegativeOriented = axisPoint(
      secondaryUnit,
      -Math.max(18, dragLength * 0.28),
    );
    const zPositive = axisPoint(normalizedZUnit, Math.max(28, dragLength * 0.72));
  ...
  const extendStroke = useCallback(
    (point: Point) => {
      ...
      for (let i = 1; i <= steps; i++) {
        ...
        if (!previewLast || dist(previewLast, nextPoint) >= previewPointStep()) {
  ...
  const endStroke = useCallback(() => {
    ...
    const requiredPointCount = toolRef.current === "shape" ? 2 : minPointsToSave;
    ...
    if (d.trim().length > 0) {
      if (shapeDraft?.strokes?.length) {
        const shapeStrokes: Stroke[] = shapeDraft.strokes.map((segment: any) => ({
          id: uid(),
        ...
        const axisGroupId = shapeStrokes.some((stroke: Stroke) => stroke.shapePreset)
          ? uid()
          : null;
        const finalizedShapeStrokes =
          axisGroupId == null
            ? shapeStrokes
            : shapeStrokes.map((stroke) => ({ ...stroke, groupId: axisGroupId }));
  ...
  const queueEraserPoints = useCallback(
    (points: Point[]) => {
      ...
      for (const point of points) {
        ...
        if (previous && dist(previous, point) < 2.5) continue;
        queue.push(point);
      }
  ...
  const eraseAlongSegment = useCallback(
    (from: Point, to: Point) => {
      const radius = activeWidthRef.current / 2;
      const step = Math.max(4, radius * 0.5);
      ...
      const sampleCount = Math.min(24, Math.ceil(distance / step));
      const samples: Point[] = [];
  ...
  const startAxisRotation = useCallback((handle: AxisRotateHandle) => {
    const selection = strokesRef.current.filter(
      (stroke) =>
        stroke.groupId === handle.groupId && stroke.axisRole === handle.axisRole,
    );
  ...
  const rotateAxisToPoint = useCallback((point: Point) => {
    ...
    const nextAngle = Math.atan2(point.y - state.origin.y, point.x - state.origin.x);
  ...
  const moveCanvasInteraction = useCallback((pageIndex: number, point: Point | null) => {
    const api = interactionApiRef.current;
      ...
      pages.map((_, pageIndex) => {
        ...
        return {
          nativeGesture: Gesture.Pan()
            ...
            .minDistance(0)
            .onStart((event) => {
              beginCanvasInteraction(pageIndex, toNativeGesturePoint(event.x, event.y));
            })
            .onUpdate((event) => {
              moveCanvasInteraction(pageIndex, toNativeGesturePoint(event.x, event.y));
            })
            .onEnd(() => {
              endCanvasInteraction(pageIndex);
            })
